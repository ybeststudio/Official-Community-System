...
CGuild::CGuild(TGuildCreateParameter& cp)
{
	...
	strlcpy(m_data.grade_array[0].grade_name, LC_STRING("Leader"), sizeof(m_data.grade_array[0].grade_name));
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] Provided Experience is larger then left Experience."));
		return false;
	}
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] This message cannot be deleted."));
	else
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	if (GetSP() < iNeededSP)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] Not enough Dragon Ghost. (%d, %d)", GetSP(), iNeededSP));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] You cannot use the guild skills yet."));
		return;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("<Guild> %d Skills used (%d, %d) to %lu", dwVnum, GetSP(), iNeededSP, pid));
	...
	case GUILD_SKILL_TELEPORT:
		...
		else
		{
			if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] This player is on channel %d. (Current channel: %d)", pcci->bChannel, g_bChannel));
				}
				else
				{
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] This player is not online."));
		}
		break;
	...
	*/
	...
	default:
	{
		if (!UnderAnyWar())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] This guild skill can only be used during war."));
			return;
		}
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("<Guild> %u Dragon ghost restored.", iSP));
	}
	return true;
}
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] Please try again later."));
		return;
	}
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] Only the guild leader can withdraw Yang."));
		return;
	}
...
void CGuild::Invite(LPCHARACTER pchInviter, LPCHARACTER pchInvitee)
{
	...
	{
		pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] The person standing opposite you cannot receive the invitation."));
		return;
	}
	...
	{
		pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] The player has declined the guild invitation."));
		return;
	}
	else if (!HasGradeAuth(GetMember(pchInviter->GetPlayerID())->grade, GUILD_AUTH_ADD_MEMBER))
	{
		pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] You do not have the authority to invite someone to join the guild."));
		return;
	}
	else if (pchInvitee->GetEmpire() != pchInviter->GetEmpire())
	{
		pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] You cannot invite players from another kingdom into the guild."));
		return;
	}
	...
	case GERR_WITHDRAWPENALTY:
		pchInviter->ChatPacket(CHAT_TYPE_INFO,
			LC_STRING("[Guild] This player can be invited again after %d day(s).",
			quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay")));
		return;
	case GERR_COMMISSIONPENALTY:
		pchInviter->ChatPacket(CHAT_TYPE_INFO,
			LC_STRING("[Guild] After the rearrangement you can invite members again after %d days.",
			quest::CQuestManager::instance().GetEventFlag("guild_disband_delay")));
		return;
	case GERR_ALREADYJOIN:	pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] This person is already a member of another Guild.")); return;
	case GERR_GUILDISFULL:	pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] The maximum guild capacity has been reached.")); return;
	case GERR_GUILD_IS_IN_WAR: pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] This guild is taking part in a battle at the moment.")); return;
	case GERR_INVITE_LIMIT: pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_STRING("<Guild> New subscription is currently restricted.")); return;
...
void CGuild::InviteAccept(LPCHARACTER pchInvitee)
{
	...
	case GERR_COMMISSIONPENALTY:
		pchInvitee->ChatPacket(CHAT_TYPE_INFO,
			...
			quest::CQuestManager::instance().GetEventFlag("guild_disband_delay")));
		return;
	case GERR_ALREADYJOIN:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] This person is already a member of another Guild.")); return;
	case GERR_GUILDISFULL:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] The maximum guild capacity has been reached.")); return;
	case GERR_GUILD_IS_IN_WAR: pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Guild] This guild is taking part in a battle at the moment.")); return;
	case GERR_INVITE_LIMIT: pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_STRING("<Guild> is currently restricted for new subscriptions.")); return;
